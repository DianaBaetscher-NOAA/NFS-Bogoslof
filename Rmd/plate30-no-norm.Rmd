...
How many individual sample replicates have < 1000 reads?

```{r looking-for-drop-out-samples}
asv_tbl_long %>%
  group_by(sample) %>%
  summarise(total_reads = sum(reads)) %>%
  filter(total_reads < 1000) %>%
  separate(sample, into = c("extraction_ID", "replicate")) %>%
  left_join(., meta, by = "extraction_ID") %>%
  filter(!is.na(sample_type)) %>%
  select(extraction_ID, replicate, total_reads, location2, location3, sample_type) %>%
  filter(sample_type != "field_blank")


```

```{r variation-across-sample-replicates}
asv_tbl_long %>%
  separate(sample, into = c("extraction_ID", "replicate")) %>%
  left_join(., meta, by = "extraction_ID") %>%
  filter(!is.na(sample_type)) %>%
  ggplot(aes(x = replicate, y = reads, fill = ASV)) +
  geom_bar(stat = "identity") +
  facet_grid(rows = vars(location3), cols = vars(extraction_ID), space = "free") +
  theme(
...
What about the normalized samples?
```{r}
plate30_norm %>%
  filter(!is.na(sample_type)) %>%
  ggplot(aes(x = replicate, y = reads, fill = ASV)) +
