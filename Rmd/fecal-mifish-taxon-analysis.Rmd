...
tax_df <- taxonomy_wo_salmonGadids %>%
  filter(X4 > 100) %>% # make sure all retained matches are >100 bp
  select(-X2, -(X5:X16)) %>%  #remove unnecessary columns
  ...
  rename(ASV=X1, perc_id=X3, length=X4, taxonomy=X17) %>% #rename headers
...
```
This doesn't include salmon/gadids at this point.
...
single_spp_seqs
```
196 ASVs that are unique to a single species.
...
to_remove_low_perc_hits <- seq_id_diff %>%
  ungroup() %>%
  group_by(ASV) %>%
  filter(diff > 2) # there are zero of those in this dataset.
...
single_genus <- keepers %>%
  anti_join(., single_spp_seqs, by = "ASV") %>% # at best, these should be genus-level matches
  group_by(ASV, genus) %>%
  tally() %>%
  ungroup() %>%
  group_by(ASV) %>%
...
single_family <- keepers %>%
  anti_join(., single_spp_seqs, by = "ASV") %>%
...
species_asvs <- single_spp_seqs %>%
  left_join(., fixed_tax_df) %>%
...
tax_df_wo_gadidSalmon <- bind_rows(single_class, single_order, single_family, single_genus, species_asvs) %>%
  select(-id_rank, -top_perc, -diff, -seq_percID)
  

```
## Now look at gadids and salmon ##

for both of these species, I'll use a 0.5% difference based on the small number of bp differences that distinguish species.
