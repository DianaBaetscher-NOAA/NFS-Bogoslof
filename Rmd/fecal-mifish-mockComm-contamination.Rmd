...
Out of curiosity, when we see Gadidae, do we always see at least a genus-level ID in the same sample?
Yes, and it only occurs in one sample.

```{r}
clean_df %>%
  group_by(ablg, taxon) %>%
  summarise(sample_reads = sum(taxon_reads)) %>%
  filter(taxon == "Gadidae")
...
```{r join-the-DNA-sample-id-with-the-ablg}
clean_df  %>%
  group_by(ablg, taxon, alternative_ID) %>%
  summarise(mean_reads = mean(taxon_reads)) %>%
  select(ablg, alternative_ID, taxon, mean_reads) %>%
  mutate(mean_reads = round(mean_reads, digits = 0)) %>%
  write_csv("Bogoslof_fecal_samples_wo_eelgrass_spp.csv")
...
clean_df  %>%
  ...
  #filter(taxon != "Gadidae") %>%
  filter(str_detect(taxon, "Oncorhynchus")) %>%
  filter(mean_reads > 100) %>%
  ggplot(aes(x = alternative_ID, y = mean_reads, fill = reorder(taxon, -mean_reads))) +
  geom_bar(stat = "identity") +
  theme_bw() +
   theme(
  ...
  labs(fill = "",
       y = "mean reads",
       x = "sample",
       title = "Oncorhynchus spp. >100 reads")
  
ggsave("pdf_outputs/salmon_in_fecal_samples_100_reads.png", height = 4, width = 7)
```
